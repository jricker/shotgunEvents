...
def process_change(sg, logger, event, args):
    ...
    #        CC.create_final(str(i), action[2])
    if action[0] == 'task_updated':
...
class Schedule():
    def __init__(self):
        self.user_list = []
    def check_change(sg, logger, event, args,self):
        meta = event['meta']
    ...
    def remove_booking(ID,self):
        if isinstance(ID, list):
            for i in ID:
                sg.delete('Booking', i )
        else:
            sg.delete('Booking', ID)
    def create_booking(user_id, project_id, start_date, end_date,self):
    ...
    def get_bookings(user_id, project_id, start_date, end_date,self):
    ...
    ##############################################################################################
    def get_all_tasks(project_id, user_id,self):
        task_list = []
        ...
        return task_list
    def process_dates(start_date, end_date,self):
        X = []
        ...
        while d <= end:
            ...
            d += delta
        return X
    def find_hole(user_id, project_id,self):
        X = []
        Y = []
        A = []
        find = []
        for i in get_all_tasks(project_id, user_id):
            X.append(i)
        for i in X:
            for x in i:
                find.append(x)
    ...
    def find_last_day(year_id, month_id,self):
        last_day = calendar.monthrange(year_id,month_id)[1]
        return last_day
    def count_days(date_list,self):
        X = []
        Y = []
        ...
        return X, Y
    def return_booking(ID,self):
        booking = sg.find('Booking', filters= [['id', 'is', ID]], fields = ['project', 'id', 'start_date', 'end_date' ])
        return booking
    def adjust_booking(ID, start_date, end_date,self):
        pass
    def create_final(user_id, project_id,self):
        for i in range(5):
            print 'adjusting schedule'
        dates = find_hole(user_id, project_id)[0][1]
        ##############################################################################################
        ...
        for i in all_overlap_bookings:
            ...
            #print i, ' THIS IS OVERLAP BOOKINGS'
            #print project['id'], ' THIS IS THE PROJECT BOOKINGS ID'
        old_bookings = []
