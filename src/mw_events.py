...
class Events():
		...
		if J['event_type'] == 'Shotgun_Asset_Change':
			if J['attribute_name'] == 'sg_asset_type':
				...
				if self.saved_event_data:
					...
					print J['meta_added'][0]['name'] # This is the name of the sequence
	...
	def update_tags(self, event_data):
		if 'meta_new_value' in event_data:
			...
			# Find the shot via the meta ID. Return ID, CODE, and ASSETS
			task = sg.find_one('Task', [['id', 'is', event_data['meta_entity_id']]], fields = ['id','code','entity.Shot.assets'])
	...
	def create_sequence(self):
		print 'placeholder'
	def create_shot(self, shot_name, shot_type, sequence_id ):
		...
		print 'placeholder'
	def create_cinematic(self, event_data = {}, asset_name = None, sequence_count = 5, shots_per_sequence = 30):
		#print 'creating CINEMATIC'
		for i in range(sequence_count):
			count = self.create_padding(data = 1+i, amount = 2) # add in the +1 on top so sc starts at 01
			sequence_name = asset_name+'_sc' + count
			#print sequence_name
			...
			for x in range(shots_per_sequence):
				...
				print shot_name
				self.create_shot(shot_name = shot_name, shot_type = 'Cinematic_Template', sequence_id = 9 )
		self.saved_event_data = False
	...
	def find_asset_template(self, event_data):
		...
		if asset_type == 'Cinematic':
			self.create_cinematic(event_data, asset_name)
